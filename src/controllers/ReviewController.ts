...
export default class ReviewController extends BaseController<IReview> {
	...
	public async getReviewsByCourseId(
		...
		if (req.query.id) {
			...
			return await this.reviewService
				.aggregateByCourseId(courseId)
				.then((reviews) => {
					return res
						.status(200)
						.send({ courseId: courseId, reviews: reviews });
				});
		}
		return res.status(400).send({
			error: "Отсутствует обязательный query-параметр 'courseId'"
		});
	}
	...
	public async addReview(
		@Body({ required: true })
		dataReview: { courseId: string; rating: number; text: string },
		@Req() req: Request,
		@Res() res: Response
	): Promise<any | Response> {
						...
						return await ReviewModel.findOne({
							...
							return CourseModel.findOne({
								...
								if (course) {
									if (existingReview) {
										return ReviewModel.updateOne(
											...
											course.rating.internal.averageScore =
												(averageScore * countReviews +
										...
										return ReviewModel.insertMany([
											...
											course.rating.internal.averageScore =
												(averageScore * countReviews +
													Number(review.rating)) /
												(countReviews + 1);
											course.rating.internal.countReviews =
												countReviews + 1;
											return CourseModel.updateOne(
												{ id: review.courseId },
												{
								...
								return res.status(404).send({
									error:
										"Не существует курса с переданным 'id'"
								});
							});
						});
					}
					return res.status(400).send({
						error: "Свойство 'text' не должно быть пустым"
					});
				}
				return res.status(400).send({
					error:
						"Свойство 'rating' должно быть целым в отрезке [1, 5]"
				});
			}
			return res.status(400).send({
				error:
	...
	public async deleteReview(
		...
		if (typeof data === "object" && data?.id) {
			...
			if (req.query.reviewId) {
				...
				return await ReviewModel.findOne({ _id: reviewId }).then(
					(review) => {
						if (review) {
							if (review.userId == userId) {
								...
								return CourseModel.findOne({
									...
									return res.status(404).send({
										error:
											"Не существует курса на который оставлен отзыв"
									});
								});
							}
							return res
								.status(400)
								.send({ error: "Нельзя удалить чужой отзыв" });
						}
